...
class Docx2Text
{
     ...
     * Construct
     * @access public
     * @param array $boolTransforms boolean values of which elements should be transformed or not
     */
    ...
    public function extract($filename = '')
    {
        if (empty($this->_document)) {
            ...
            PhpdocxLogger::logger('There is no content from document.xml', 'fatal');
        }
    ...
    {
        if (file_exists(dirname(__FILE__) . '/DOCXStructureTemplate.php') && $filename instanceof DOCXStructure) {
            $this->docx = $filename;
        } else {
            $this->docx = new DOCXStructure();
            $this->docx->parseDocx($filename);
        }

        $this->_document = $this->docx->getContent('word/document.xml');
    }
     ...
     *
     * @access public
     * @param $filename the name of the file which is going to write
     * @param $data the data of the file
     */
    public function writeFile($filename, $data)
    {
        if (!$handle = fopen($filename, 'w+')) {
            PhpdocxLogger::logger('Cannot open file ($filename)', 'fatal');
        }
        // Write $data to our opened file.
        if (fwrite($handle, $data) === FALSE) {
            PhpdocxLogger::logger('Cannot write to file ($filename)', 'fatal');
        }
        fclose($handle);
    }
    ...
    private function loadEndNote()
    {
        if (empty($this->endnotes)) {
            if (empty($this->_endnote)) {
                $this->_endnote = $this->docx->getContent('word/endnotes.xml');
            }
            if (!empty($this->_endnote)) {
    ...
    private function loadFootNote()
    {
        if (empty($this->footnotes)) {
            if (empty($this->_footnote)) {
                $this->_footnote = $this->docx->getContent('word/footnotes.xml');
            }
            if (!empty($this->_footnote)) {
                $domDocument = new DOMDocument();
                if (PHP_VERSION_ID < 80000) {
    ...
    private function listNumbering()
    {
        ...
        $this->_numbering = $this->docx->getContent('word/numbering.xml');
    ...
    private function loadRelations()
    {
        if (empty($this->relations)) {
            if (empty($this->_relations)) {
                $this->_relations = $this->docx->getContent('word/_rels/document.xml.rels');
            }
            $domDocument = new DOMDocument();
            if (PHP_VERSION_ID < 80000) {
                $optionEntityLoader = libxml_disable_entity_loader(true);
     ...
     *
     * @access private
     * @param DOMNode $node is an object DOMNode
     */
