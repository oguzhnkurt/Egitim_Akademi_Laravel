...
class HTML2WordML
{
     ...
     * @var int
     */
    public static $scaleImageFactor = 7200;
    ...
    public function render($html, $options, $docx = null)
    {
        ...
        if (isset($options['streamContext'])) {
            $this->streamContext = $options['streamContext'];
        }
        ...
        if (isset($options['wordStyles']) && is_array($options['wordStyles'])) {
            ...
            } else { //FIXME change this when "tableStyle" and "paragraphStyle" disapears
        ...
        // clean
        if (file_exists(dirname(__FILE__) . '/HTMLExtended.php') && self::$htmlExtended) {
            $this->wordML = str_replace(' ', ' ', $this->wordML);
        }
    ...
    {
        require_once dirname(__FILE__) . '/DOMPDF_lib.php';
    ...
    private function _render($nodo, $depth = 0)
    {
        ...
        switch ($nodo['nodeName']) {
            ...
            case 'dd':
                ...
                if (file_exists(dirname(__FILE__) . '/HTMLExtended.php') && self::$htmlExtended && ($nodo['nodeName'] == 'figcaption' || $nodo['nodeName'] == 'caption') && isset($nodo['attributes']['data-label']) && $nodo['attributes']['data-label'] != '') {
                ...
                if ($this->customListStyles) {
                    ...
                    if ($listLevel == 0) {
                        ...
                        if (!empty($currentListStyle)) {
                            self::$currentCustomList = CreateDocx::uniqueNumberId(999, 32766);
                            self::$currentListStyle = $currentListStyle;
                            self::$customLists[] = array('name' => $currentListStyle . '_' . self::$currentCustomList, 'id' => self::$currentCustomList, 'attributes' => array(array('start' => $nodo['attributes']['start'], 'listLevel' => $listLevel)));
                        } else {
                            self::$currentCustomList = null;
                            self::$currentListStyle = null;
                        }
                        ...
                        if (file_exists(dirname(__FILE__) . '/HTMLExtended.php') && self::$htmlExtended) {
                            ...
                            if (isset($nodo['attributes']) && isset($nodo['attributes']['class'])) {
                                ...
                                if (!is_null($currentListStyle)) {
                                    self::$currentCustomListLvlOverride = CreateDocx::uniqueNumberId(999, 32766);
            ...
            case 'a':
                ...
                if (isset($nodo['attributes']['href']) && $nodo['attributes']['href'] != '') {
                    $aId = 'rId' . uniqid((string)mt_rand(999, 9999));
                    if ($this->openPs) {
                        if ($nodo['attributes']['href'][0] != '#') {
                            $this->openLinks = true;
                            $this->wordML .= '<w:hyperlink r:id="' . $aId . '" w:history="1" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships">';
                            $xmlUtilities = new XmlUtilities();
                            self::$linkTargets[$aId] = $xmlUtilities->parseAndCleanTextString($this->parseURL($nodo['attributes']['href']));
                ...
                if ($this->openSelect) {
                    if (count($this->selectOptions) < 25) {
                        if ($this->selectedOption == 1) {
                            $xmlUtilities = new XmlUtilities();
                            array_unshift($this->selectOptions, $xmlUtilities->parseAndCleanTextString($nodo['nodeValue']));
                        } else {
                            $xmlUtilities = new XmlUtilities();
                            $this->selectOptions[] = $xmlUtilities->parseAndCleanTextString($nodo['nodeValue']);
                        }
                    } else {
                        echo 'The 25 limit of items that Word has stablished for a dropdown list has been exceeded' . PHP_EOL;
                    }
                } else if ($this->openTextArea) {
                    $xmlUtilities = new XmlUtilities();
                    $this->textArea = $xmlUtilities->parseAndCleanTextString($nodo['nodeValue']);
                } else if ($this->isSvgTag) {
                    // do not add the text
                } else {
                    if ($this->openPs) {
                        $this->wordML .= '<w:r>';
                    ...
                    $xmlUtilities = new XmlUtilities();
                    $this->wordML .= '<w:t xml:space="preserve">' . $xmlUtilities->parseAndCleanTextString($nodo['nodeValue']) . '</w:t>';
                    $this->wordML .= '</w:r>';
            ...
            case 'svg':
                $this->wordML .= $this->closePreviousTags($depth, $nodo['nodeName']);
                // default descr value
                $descr = 'image' . uniqid((string)mt_rand(999, 9999));
                        ...
                        if ($extension == 'svg') {
                            if (extension_loaded('imagick')) {
                                ...
                                if (!is_null($this->streamContext)) {
                                    $svg = @file_get_contents($this->parseURL($nodo['attributes']['src']), false, $this->streamContext);
                                } else {
                                ...
                                if (!$svg) {
                                    if (!is_null($this->streamContext)) {
                                        $svg = @file_get_contents(str_replace('http://.', '', $nodo['attributes']['src']), false, $this->streamContext);
                                    } else {
                            ...
                            if (!is_null($this->streamContext)) {
                                $photo = @file_get_contents($this->parseURL($nodo['attributes']['src']), false, $this->streamContext);
                            } else {
                            ...
                            if (!$photo) {
                                if (!is_null($this->streamContext)) {
                                    $photo = @file_get_contents(str_replace('http://.', '', $nodo['attributes']['src']), false, $this->streamContext);
                                } else {
                ...
                $imgId = 'rId' . uniqid((string)mt_rand(999, 9999));
                $tempName = 'name' . uniqid((string)mt_rand(999, 9999));
                ...
                if (file_exists(dirname(__FILE__) . '/HTMLExtended.php') && self::$htmlExtended) {
                    ...
                    if (isset($properties['max_width'])) {
                        $properties['width'] = str_replace(' ', '', $properties['max_width']);
                        $imageSize['width'] = $this->CSSUnits2Pixels($properties['max_width'], $properties['font_size']);
                    }
                    if (isset($properties['max_height'])) {
                        $properties['height'] = str_replace(' ', '', $properties['max_height']);
                        $imageSize['height'] = $this->CSSUnits2Pixels($properties['max_height'], $properties['font_size']);
                    }
                    $cMax = $this->getImageDimensions($size, $imageSize);
                    if ($cMax[0] < $c[0]) {
                        $c[0] = $cMax[0];
                    }
                    if ($cMax[1] < $c[1]) {
                    ...
                    // percent values
                    $applyPercentValue = false;
                    if (isset($properties['max_width']) && preg_match('/^(-?\d*\.?\d*)(%|em|pt|px)?$/i', trim($properties['max_width']), $match)) {
                        $match[1] = (strpos($match[1], '.') === 0 ? '0' : '') . $match[1];
                        $match[2] = empty($match[2]) ? '' : $match[2];

                        if ($match[2] == '%') {
                            $imageSize['width'] = $size[0] * ($match[1] / 100);
                            $applyPercentValue = true;
                        }
                    }
                    if (isset($properties['max_height']) && preg_match('/^(-?\d*\.?\d*)(%|em|pt|px)?$/i', trim($properties['max_height']), $match)) {
                        $match[1] = (strpos($match[1], '.') === 0 ? '0' : '') . $match[1];
                        $match[2] = empty($match[2]) ? '' : $match[2];

                        if ($match[2] == '%') {
                            $imageSize['height'] = $size[1] * ($match[1] / 100);
                            $applyPercentValue = true;
                        }
                    }
                    if ($applyPercentValue) {
                        $cMax = $this->getImageDimensions($size, $imageSize);
                        if ($cMax[0] < $c[0]) {
                            $c[0] = $cMax[0];
                        }
                ...
                // link wrapping img content
                $docPrContent = '';
                if ($this->openLinks && count(self::$linkTargets) > 0) {
                    $linkTargetsImg = array_slice(self::$linkTargets, -1);
                    $hlinkId = key($linkTargetsImg);
                    if (!empty($hlinkId)) {
                        $docPrContent = '<a:hlinkClick r:id="'.$hlinkId.'" xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main"/>';
                    }
                }

                // alt attribute
                $descr = rawurlencode($descr);
                if (isset($nodo['attributes']['alt'])) {
                    $descr = htmlspecialchars($nodo['attributes']['alt']);
                }

                // positioning
                if (isset($properties['float']) && ($properties['float'] == 'left' || $properties['float'] == 'right')) {
                    $docPr = rand(99999, 99999999);
                    $this->wordML .= '<w:drawing xmlns:wp="http://schemas.openxmlformats.org/drawingml/2006/wordprocessingDrawing" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><wp:anchor distT="' . $distance['top'] . '" distB="' . $distance['bottom'] . '" distL="' . $distance['left'] . '" distR="' . $distance['right'] . '" simplePos="0" relativeHeight="251658240" behindDoc="0" locked="0" layoutInCell="1" allowOverlap="0"><wp:simplePos x="0" y="0" />';
                    ...
                    $this->wordML .= '<wp:wrapSquare wrapText="bothSides" /><wp:docPr id="' . $docPr . '" name="' . $tempName . '" descr="' . $descr . '">'.$docPrContent.'</wp:docPr>';
                    ...
                    $this->wordML .= '<a:graphicData uri="http://schemas.openxmlformats.org/drawingml/2006/picture"><pic:pic xmlns:pic="http://schemas.openxmlformats.org/drawingml/2006/picture"><pic:nvPicPr><pic:cNvPr id="0" name="' . $descr . '"/><pic:cNvPicPr/></pic:nvPicPr><pic:blipFill>';
                    if ($this->downloadImages) {
                    ...
                    $this->wordML .= '<w:drawing xmlns:wp="http://schemas.openxmlformats.org/drawingml/2006/wordprocessingDrawing" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><wp:inline distT="' . $distance['top'] . '" distB="' . $distance['bottom'] . '" distL="' . $distance['left'] . '" distR="' . $distance['right'] . '"><wp:extent cx="' . $cx . '" cy="' . $cy . '" />';
                    $this->wordML .= '<wp:effectExtent b="'.$distance['bottom'].'" l="'.$distance['left'].'" r="'.$distance['right'].'" t="'.$distance['top'].'"/>';
                    $this->wordML .= '<wp:docPr id="' . rand(99999, 99999999) . '" name="' . $tempName . '" descr="' . $descr . '">'.$docPrContent.'</wp:docPr><wp:cNvGraphicFramePr><a:graphicFrameLocks xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main" noChangeAspect="1" /></wp:cNvGraphicFramePr>';
                    ...
                    $this->wordML .= '<pic:nvPicPr><pic:cNvPr id="0" name="' . $descr . '"/><pic:cNvPicPr/></pic:nvPicPr><pic:blipFill>';
                    if ($this->downloadImages) {
                        $this->wordML .= '<a:blip r:embed="' . $imgId . '" cstate="print"/>';
                    } else {
                        $this->wordML .= '<a:blip r:link="' . $imgId . '" cstate="print"/>';
            ...
            case 'samp':
                ...
                if ($titArray[0] == 'phpdocx') {
                    if ($titArray[1] == 'footnote') {
                        ...
                        if (isset($properties['color']) && $properties['color'] != '' && is_array($properties['color'])) {
            ...
            default:
                ...
                if (file_exists(dirname(__FILE__) . '/HTMLExtended.php') && self::$htmlExtended) {
                    ...
                    if (array_key_exists($nodo['nodeName'], $extendedTags)) {
                        if ($extendedTags[$nodo['nodeName']] == 'addHeader' || $extendedTags[$nodo['nodeName']] == 'addFooter') {
                            ...
                            if (!empty($wordFragmentHtmlContent)) {
                                ...
                                if ($extendedTags[$nodo['nodeName']] == 'addHeader') {
                                    ...
                                    // keep current values to avoid removing them in embedHTML
                                    $currentLinkImages = self::$linkImages;
                                    $currentLinkTargets = self::$linkTargets;
                                    $wordFragmentContent->embedHTML($wordFragmentHtmlContent, array('useHTMLExtended' => true));
                                    self::$linkImages = array_merge(self::$linkImages, $currentLinkImages);
                                    self::$linkTargets = array_merge(self::$linkTargets, $currentLinkTargets);
                            ...
                            if (!isset($attributesContent['type'])) {
                                $attributesContent['type'] = 'numerical';
                            }
                            ...
                            if ($this->openPs) {
                                $wordFragmentContentCleaned = preg_replace('/__PHX=__[A-Z]+__/', '', (string)$wordFragmentContent->inlineWordML());
                                $this->wordML .= $wordFragmentContentCleaned;
                            } else {
                                $wordFragmentContentCleaned = preg_replace('/__PHX=__[A-Z]+__/', '', (string)$wordFragmentContent);
                                $this->wordML .= $wordFragmentContentCleaned;
                            }
                            ...
                            $wordFragmentContentCleaned = preg_replace('/__PHX=__[A-Z]+__/', '', (string)$wordFragmentContent);
                            $this->wordML .= $wordFragmentContentCleaned;
                        } else if ($extendedTags[$nodo['nodeName']] == 'modifyPageLayout') {
                            $attributesContent = array();
                            foreach ($nodo['attributes'] as $key => $value ) {
                                $attributesContent[str_replace('data-', '', $key)] = $value;
                            }
                        ...
                        } else if ($extendedTags[$nodo['nodeName']] == 'title' && !empty($nodo['nodeValue'])) {
                            $this->propertiesDocument['title'] = $nodo['nodeValue'];
                        } else if ($extendedTags[$nodo['nodeName']] == 'meta') {
                            $validMetas = array(
                                'subject' => 'subject',
                                'creator' => 'creator',
                                'keywords' => 'keywords',
            ...
            switch ($tag) {
                ...
                case 'tr':
                    ...
                    if (strpos($this->wordML, '#<w:gridCol/>#') !== false) {
                        ...
                        } else if (@$this->tableGrid[$this->openTable][$row][$row][2]['width'] !== null) {
                ...
                case 'select':
                    $this->openSelect = false;
                    $dropdownId = uniqid((string)mt_rand(999, 9999));
    ...
    private function listType($type = array(1, 2))
    {
        ...
        for ($j = $counter; $j >= ($this->_level - 1); $j--) {
            if (isset($this->openTags[$j]) && $this->openTags[$j] == 'ul') {
                $num = $type[0];
                break;
            } else if (isset($this->openTags[$j]) && $this->openTags[$j] == 'ol') {
                $num = $type[1];
                break;
            }
        }
    ...
    private function generatePPr($properties, $level = '', $attributes = array(), $nodeName = false)
    {
        ...
        if (!$this->strictWordStyles) {
            ...
            foreach (self::$borders as $key => $value) {
                if (isset($properties['border_' . $value . '_style']) && $properties['border_' . $value . '_style'] != 'none') {
                    if (!(isset($properties['border_' . $value . '_color']))) {
                        $properties['border_' . $value . '_color'] = array('hex' => 'transparent');
                    }
        ...
        if ((isset($attributes['dir']) && array_key_exists($attributes['dir'], self::$text_direction)) || (isset($attributes['dir']) && array_key_exists($attributes['dir'], self::$text_direction_lowercase)) || (isset($properties['direction']) && array_key_exists($properties['direction'], self::$text_direction))) {
            ...
            } else if (isset($attributes['dir']) && array_key_exists($attributes['dir'], self::$text_direction_lowercase)) {
                $stringPPr .= '<w:textDirection w:val="' . self::$text_direction_lowercase[$attributes['dir']] . '" />';
            } else if (isset($properties['direction']) && array_key_exists($properties['direction'], self::$text_direction)) {
        ...
        if (is_numeric($level)) {
            $addHeading  = true;
            if (isset($properties['data_outline_level']) && $properties['data_outline_level'] == 'none') {
                $addHeading  = false;
            }
     ...
     * w:strike (text-decoration="line-through|double-line-through"),
     * w:color (color),
     * w:position (vertical-align!="baseline")(font-size),
     * w:spacing (letter-spacing)
     * w:sz (font-size),
     * w:u (text-decoration="underline|dashed|dotted|double|solid|wavy")
     * w:w (font-streched)
     *
     * @access private
     * @param array $properties
    ...
    private function generateRPr($properties, $level = '', $attributes = array(), $nodeName = false)
    {
        ...
        if ((isset($properties['text_decoration']) && $properties['text_decoration'] == 'line-through') || (isset($properties['text_decoration']) && strstr($properties['text_decoration'], 'line-through') !== false)) {
            $stringRPr .='<w:strike />';
        }
        if ((isset($properties['text_decoration']) && $properties['text_decoration'] == 'double-line-through') || (isset($properties['text_decoration']) && strstr($properties['text_decoration'], 'double-line-through') !== false)) {
            $stringRPr .='<w:dstrike />';
        }
        if (!$this->strictWordStyles) {
            if ($this->addDefaultStyles) {
                ...
                if (isset($properties['font_size']) && $properties['font_size'] != '') {
                    if (!isset($properties['data_font_size']) || (isset($properties['data_font_size']) && $properties['data_font_size'] != 'initial')) {
        ...
        if (($this->openLinks && isset($properties['text_decoration']) && $properties['text_decoration'] != 'none') || (isset($properties['text_decoration']) && $properties['text_decoration'] == 'underline') || (isset($properties['text_decoration']) && strstr($properties['text_decoration'], 'underline') !== false)) {
            ...
            if ($properties['text_decoration'] != 'line-through' && $properties['text_decoration'] != 'double-line-through' && !empty($textDecorationColor)) {
        ...
        if (!$this->strictWordStyles) {
            if (isset($properties['background_color']) && $properties['background_color'] != '' && is_array($properties['background_color'])) {
                $color = $properties['background_color'];
                $color = $this->wordMLColor($color);
        ...
        if (isset($properties['vertical_align']) && $properties['vertical_align'] != 'baseline') {
            ...
            } else if ($properties['vertical_align'] == 'supper' || $properties['vertical_align'] == 'super') {
     ...
     * w:tblBorders (border-width) (border-[top|left|bottom|right]-style)(border-[top|left|bottom|right]-color)(border-[top|left|bottom|right]-width)
    ...
    private function generateTblPr($properties, $attributes)
    {
        ...
        if (!$this->strictWordStyles) {
            ...
            if (isset($properties['table_layout'])) {
                ...
                } else if ($properties['table_layout'] == 'autofit') {
                    $stringTblPr .= '<w:tblLayout w:type="autofit"></w:tblLayout>';
                }
            }
        }
    ...
    private function generateTcPr($properties, $attributes, $colspan, $rowspan, $firstRow)
    {
        ...
        if (!$this->strictWordStyles) {
            if ($this->tableStyle == '') {
                $sTemp = '';
                foreach (self::$borders as $key => $value) {
                    if (!empty($properties['border_' . $value . '_width']) && (!isset($properties['border_' . $value . '_style']) || (isset($properties['border_' . $value . '_style']) && $properties['border_' . $value . '_style'] != 'none'))) {
            ...
            // w:noWrap
            if (isset($properties['white_space']) && $properties['white_space'] == 'nowrap') {
                $stringTcPr .= '<w:noWrap/>';
            }
        }
        ...
        if (file_exists(dirname(__FILE__) . '/HTMLExtended.php') && self::$htmlExtended) {
            ...
            if (isset($properties['data_fit_text']) && $properties['data_fit_text'] == 'always') {
                $stringTcPr .= '<w:tcFitText w:val="true"/>';
            }
        }
    ...
    private function generateListPr($properties, $level = '', $attributes = array(), $nodeName = false)
    {
        ...
        if (isset($properties['list_style_type']) && $properties['list_style_type'] == 'none') {
            ...
            if (file_exists(dirname(__FILE__) . '/HTMLExtended.php') && self::$htmlExtended) {
                ...
                } else if (!empty(self::$currentCustomList)) {
        ...
        if (isset($properties['font_weight']) && ($properties['font_weight'] == 'bold' || $properties['font_weight'] == 'bolder')) {
    ...
    {
        return (int) round(((float)$size) * 5 / 0.75);
    }
     ...
     * @param bool $fontSize Font size, if applicable
     * @return array
     */
    ...
    {
        if (!preg_match('/^(-?\d*\.?\d*)(%|em|pt|in|cm|px)?$/i', trim($value), $match)) {
            return;
        }
        ...
        switch ($match[2]) {
            ...
            case 'em':
                ...
                break;
            case 'pt': // 1pt ~= 1.333px with the default HTML2WordML::$scaleImageFactor value (7200)
                $pixels = ceil($match[1] / 0.75);
                break;
            case 'in': // 1in ~= 96px with the default HTML2WordML::$scaleImageFactor value (7200)
                $pixels = ceil($match[1] * 96);
                break;
            case 'cm': // 1cm ~= 38px with the default HTML2WordML::$scaleImageFactor value (7200)
                $pixels = ceil($match[1] * 38);
                break;
            case 'px':
            default: // if no unit we asume is given in pixels
                $pixels = $match[1];
    ...
    private function generateImageRPr($properties, $height = 0)
    {
        ...
        $ptHeight = ceil(0.58 * $height / self::$scaleImageFactor);
    ...
    private function generateHTMLExtendedNote($node, $nodeName, $attributesContent)
    {
        ...
        if ($nodeName == 'addComment') {
            ...
            $textNoteMark = '';
            $textNoteReferenceMark = '';
            $textNoteTargetContent = 'textComment';
            $textNoteTarget = 'textComments';
            $target = 'comment';
            ...
            $textNoteMark = 'phpdocx_endnote_mark';
            $textNoteReferenceMark = 'phpdocx_endnote_reference_mark';
            $textNoteTargetContent = 'textEndnote';
            $textNoteTarget = 'textEndnotes';
            $target = 'endnote';
            ...
            $textNoteMark = 'phpdocx_footnote_mark';
            $textNoteReferenceMark = 'phpdocx_footnote_reference_mark';
            $textNoteTargetContent = 'textFootnote';
            $textNoteTarget = 'textFootnotes';
            $target = 'footnote';
        ...
        foreach ($contents as $content) {
            if (isset($content->tagName) && $content->tagName == $textDocumentScope) {
                // get textdocument
                ...
                // get text note. Multiple notes can exist
                $contentText = $node['inheritContents']->getElementsByTagName($textNoteScope);
                $wordFragmentTextContents = array();
                foreach ($contentText as $contentTextElement)  {
                    $wordFragmentTextContent = new WordFragment($this->docx, $target);
                    $wordFragmentHtmlContent = '';
                    if (!empty($this->CSSdocument)) {
                        $wordFragmentHtmlContent = $this->CSSdocument . $contentTextElement->ownerDocument->saveXML($contentTextElement);
                    } else {
                        $wordFragmentHtmlContent = $contentTextElement->ownerDocument->saveXML($contentTextElement);
                    }
                    // keep current values to avoid removing them in embedHTML
                    $currentLinkImages = self::$linkImages;
                    $currentLinkTargets = self::$linkTargets;
                    ...
                    self::$linkImages = array_merge(self::$linkImages, $currentLinkImages);
                    self::$linkTargets = array_merge(self::$linkTargets, $currentLinkTargets);
                    $newContentToBeAdded = array($textNoteTargetContent => $wordFragmentTextContent);
                    $newContentToBeAdded = array_merge($newContentToBeAdded, $options);
                    if ($contentTextElement->hasAttribute('data-author')) {
                        $newContentToBeAdded['author'] = $contentTextElement->getAttribute('data-author');
                    }
                    if ($contentTextElement->hasAttribute('data-initials')) {
                        $newContentToBeAdded['initials'] = $contentTextElement->getAttribute('data-initials');
                    }
                    if ($contentTextElement->hasAttribute('data-date')) {
                        $newContentToBeAdded['date'] = $contentTextElement->getAttribute('data-date');
                    }
                    if ($contentTextElement->hasAttribute('data-completed')) {
                        $newContentToBeAdded['completed'] = $contentTextElement->getAttribute('data-completed');
                    }
                    if ($contentTextElement->hasAttribute('data-paraId')) {
                        $newContentToBeAdded['paraId'] = $contentTextElement->getAttribute('data-paraId');
                    }

                    // get marks for endnotes and footnotes
                    if (!empty($textNoteMark)) {
                        // text note mark
                        $contentMark = $node['inheritContents']->getElementsByTagName($textNoteMark);
                        // the note mark tag is optional
                        if ($contentMark->length > 0) {
                            if ($contentMark->item(0)->hasAttributes()) {
                                $stylesMark = array();
                                foreach ($contentMark->item(0)->attributes as $attributeMark) {
                                    $styleName = $attributeMark->nodeName;
                                    // normalize style names
                                    if ($styleName == 'data-custommark') {
                                        $styleName = 'data-customMark';
                                    }
                                    if ($styleName == 'data-fontsize') {
                                        $styleName = 'data-fontSize';
                                    }
                                    if ($styleName == 'data-highlightcolor') {
                                        $styleName = 'data-highlightColor';
                                    }
                                    if ($styleName == 'data-backgroundcolor') {
                                        $styleName = 'data-backgroundColor';
                                    }
                                    $stylesMark[str_replace('data-', '', $styleName)] = $attributeMark->nodeValue;
                                }
                                if ($textNoteMark == 'phpdocx_endnote_mark') {
                                    $newContentToBeAdded['endnoteMark'] = $stylesMark;
                                } else if ($textNoteMark == 'phpdocx_footnote_mark') {
                                    $newContentToBeAdded['footnoteMark'] = $stylesMark;
                                }
                            }
                        }
                    }
                    if (!empty($textNoteReferenceMark)) {
                        // text note referenc mark
                        $contentReferenceMark = $node['inheritContents']->getElementsByTagName($textNoteReferenceMark);
                        // the note reference mark tag is optional
                        if ($contentReferenceMark->length > 0) {
                            if ($contentReferenceMark->item(0)->hasAttributes()) {
                                $stylesReferenceMark = array();
                                foreach ($contentReferenceMark->item(0)->attributes as $attributeMark) {
                                    $styleName = $attributeMark->nodeName;
                                    // normalize style names
                                    if ($styleName == 'data-fontsize') {
                                        $styleName = 'data-fontSize';
                                    ...
                                    $stylesReferenceMark[str_replace('data-', '', $styleName)] = $attributeMark->nodeValue;
                                }
                                $newContentToBeAdded['referenceMark'] = $stylesReferenceMark;
                            }
                        }
                    }

                    $wordFragmentTextContents[] = $newContentToBeAdded;
                }

                $options[$textNoteTarget] = $wordFragmentTextContents;

                // normalize key names
                if (isset($options['pstyle'])) {
                    $options['pStyle'] = $options['pstyle'];
                }
                if (isset($options['rstyle'])) {
                    $options['rStyle'] = $options['rstyle'];
                }
                ...
                if (!strstr($wordMLContent, '<w:p')) {
                    if (isset($attributesContent['type']) && $attributesContent['type'] == 'inline') {
            ...
            } else if (isset($content->tagName) && $content->tagName == $textNoteScope) {
                continue;
            } else {
            ...
            if (!isset($attributesContent['type']) || (isset($attributesContent['type']) && $attributesContent['type'] != 'inline')) {
     ...
     * @param string $font_size Font size, if applicable
     * @return mixed
     */
