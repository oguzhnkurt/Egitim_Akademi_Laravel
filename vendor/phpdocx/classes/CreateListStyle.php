...
class CreateListStyle
{
    ...
    public function addListStyle($name, $styleOptions, $addLvlOverride = false)
    {
                    ...
                    <w:numPicBullet w:numPicBulletId="'.$styleOptions[$k]['image']['rId'].'">
                        <w:pict>
                            <v:shape style="width:'.$styleOptions[$k]['image']['width'].'pt;height:'.$styleOptions[$k]['image']['height'].'pt">
                                <v:imagedata o:title="image" r:id="rId'.$styleOptions[$k]['image']['rId'].'" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"/>
                            </v:shape>
                        </w:pict>
                    </w:numPicBullet>
                ';
        ...
        for ($k = 0; $k < 9; $k++) {
            ...
            if (isset($styleOptions[$k]['typeCustom'])) {
                $baseList .= '<mc:AlternateContent><mc:Choice Requires="w14"><w:numFmt w:format="' . $styleOptions[$k]['typeCustom'] . '" w:val="custom"/></mc:Choice><mc:Fallback><w:numFmt w:val="' . $styleOptions[$k]['type'] . '"/></mc:Fallback></mc:AlternateContent>';
            } else {
                $baseList .= '<w:numFmt w:val="' . $styleOptions[$k]['type'] . '"/>';
            }
            if (isset($styleOptions[$k]['suff'])) {
                $baseList .= '<w:suff w:val="' . $styleOptions[$k]['suff'] . '"/>';
            }
