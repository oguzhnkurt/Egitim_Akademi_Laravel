...
class DOCXStructure
{
    ...
    public function getDocx($format) {
        return $this->docxStructure;
    }

    /**
     * Setter docxStructure
     *
     * @param array $docxContents array
     * @return mixed DOCX structure
     */
    public function setDocx($docxContents) {
        $this->docxStructure = $docxContents;
    }
    ...
    public function getContentByType($type)
    {
        ...
        switch ($type) {
            case 'comments':
                $queryXpath = '//xmlns:Override[@ContentType="application/vnd.openxmlformats-officedocument.wordprocessingml.comments+xml"]';
                break;
            case 'endnotes':
                $queryXpath = '//xmlns:Override[@ContentType="application/vnd.openxmlformats-officedocument.wordprocessingml.endnotes+xml"]';
                break;
            case 'footnotes':
                $queryXpath = '//xmlns:Override[@ContentType="application/vnd.openxmlformats-officedocument.wordprocessingml.footnotes+xml"]';
                break;
            case 'headers':
    ...
    public function parseDocx($path)
    {
        ...
        if ($zip->open($path) === TRUE) {
            ...
            throw new Exception('Error while trying to open ' . $path . ' as a ZIP file');
        }
        ...
        // check if there's at least a section. Otherwise create it, as it's needed to the correct workflow of working with DOCX documents
        if (isset($this->docxStructure['word/document.xml'])) {
            $documentContent = $this->docxStructure['word/document.xml'];
            $xmlUtilities = new XmlUtilities();
            $documentContentDOM = $xmlUtilities->generateDomDocument($documentContent);
            $documentContentXpath = new DOMXPath($documentContentDOM);
            $documentContentXpath->registerNamespace('w', 'http://schemas.openxmlformats.org/wordprocessingml/2006/main');

            $querySectPr = '//w:body/w:sectPr';
            $sectPrNodes = $documentContentXpath->query($querySectPr);
            if ($sectPrNodes->length < 1) {
                // there's no section, add it before </w:body>
                $sectPrContent = '<w:sectPr><w:pgSz w:w="11906" w:h="16838"/><w:pgMar w:top="1417" w:right="1701" w:bottom="1417" w:left="1701" w:header="708" w:footer="708" w:gutter="0"/><w:cols w:space="708"/><w:docGrid w:linePitch="360"/></w:sectPr>';
                $this->docxStructure['word/document.xml'] = str_replace('</w:body>', $sectPrContent . '</w:body>', $documentContent);
            }

            // move strict variant namespaces to transitional
            $documentContentXpath = new DOMXPath($documentContentDOM);
            $documentContentXpath->registerNamespace('w', 'http://purl.oclc.org/ooxml/wordprocessingml/main');
            $xmlnsNodesDocument = $documentContentXpath->query('//w:document');
            if ($xmlnsNodesDocument->length > 0) {
                $xmlnsNodeDocument = $xmlnsNodesDocument->item(0);
                if ($xmlnsNodeDocument->hasAttribute('w:conformance') && $xmlnsNodeDocument->getAttribute('w:conformance') == 'strict') {
                    foreach ($this->docxStructure as $docxStructureKey => $docxStructureValue) {
                        $xmlFile = substr_compare($docxStructureKey, '.xml', -strlen('.xml'));
                        $relsFile = substr_compare($docxStructureKey, '.rels', -strlen('.rels'));
                        if ($xmlFile === 0 || $relsFile === 0) {
                            $docxStructureValue = str_replace('http://purl.oclc.org/ooxml/officeDocument', 'http://schemas.openxmlformats.org/officeDocument/2006', $docxStructureValue);
                            $docxStructureValue = str_replace('http://purl.oclc.org/ooxml/wordprocessingml', 'http://schemas.openxmlformats.org/wordprocessingml/2006', $docxStructureValue);
                            $docxStructureValue = str_replace('http://purl.oclc.org/ooxml/drawingml', 'http://schemas.openxmlformats.org/drawingml/2006', $docxStructureValue);

                            $this->docxStructure[$docxStructureKey] = $docxStructureValue;
                        }
                    }
                }
            }
     ...
     * @throws Exception error generating the file
     */
    public function saveDocx($path, $forceFile = false) {
        // check if the path has as extension
        if (substr($path, -5) !== '.docx' && substr($path, -5) !== '.docm') {
            $path .= '.docx';
        }

        // return the structure object instead of creating the file
        if (file_exists(dirname(__FILE__) . '/ZipStream.php') && CreateDocx::$returnDocxStructure == true) {
            return $this;
        }

        // check if stream mode is true
        if (file_exists(dirname(__FILE__) . '/ZipStream.php') && CreateDocx::$streamMode === true && $forceFile === false) {
