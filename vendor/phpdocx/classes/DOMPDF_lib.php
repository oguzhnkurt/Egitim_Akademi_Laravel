...
class FrameParser_Tree {
  ...
  protected function _build_tree_r(DomNode $node) {
    if (file_exists(dirname(__FILE__) . '/HTMLExtended.php') && HTML2WordML::$htmlExtended) {
      self::$_HIDDEN_TAGS = array("area", "base", "basefont", "style", "colgroup", "noembed", "noscript", "param", "#comment", 'script');
    }
...
class PARSERHTML {
   ...
   * @return array
   */
  public function getterCSSFontFace() {
    return $this->_css->get_font_face_data();
  }
  ...
  function load_html($str, $encoding = null) {
    ...
    if (PHP_VERSION_ID >= 80200) {
        $str = @mb_encode_numericentity($str, array(0x80, 0xfffffff, 0, 0xfffffff), 'UTF-8');
    } else {
  ...
  private function _load_html($str){
    ...
    $str = preg_replace_callback(
      '/>(\s*$\s*)</m',
      array(get_class($this), '_clean_html_load_html'),
      $str
    );
  ...
  protected function _clean_html_load_html($matches) {
    return strpos('$matches[0]', ' ') === false?'><':'> <';
  }
  ...
  protected function _process_html($filter = '*') {
    ...
    if (CreateDocx::$baseCSSHTML != null && file_exists(dirname(__FILE__) . '/HTMLExtended.php')) {
      $this->_css->load_css(CreateDocx::$baseCSSHTML);
    }
    ...
    foreach ($styles as $style) {
      ...
      if (HTML2WordML::$cssEntityDecode == true) {
  ...
  private function getAttributes($node){
    ...
    switch ($node->nodeName) {
      ...
      case 'meta':
        if (file_exists(dirname(__FILE__) . '/HTMLExtended.php') && HTML2WordML::$htmlExtended) {
          $aRet['name'] = (string)$node->getAttribute('name');
          $aRet['content'] = (string)$node->getAttribute('content');
        }
        break;
      case 'ol':
      ...
      case 'p':
        $aRet['dir'] = (string)$node->getAttribute('dir');
        $aRet['lang'] = (string)$node->getAttribute('lang');
        break;
      case 'span':
        $aRet['lang'] = (string)$node->getAttribute('lang');
        break;
...
class StyleParser {
  ...
  function __construct(StyleParsersheet $stylesheet) {
    ...
    // CSS Extended
    if (file_exists(dirname(__FILE__) . '/HTMLExtended.php') && HTML2WordML::$htmlExtended) {
      $cssExtendedTags = HTMLExtended::getCSSExtendedStyles();
      foreach ($cssExtendedTags as $cssExtendedTagsKey => $cssExtendedTagsValue) {
        $prop = str_replace('-', '_', $cssExtendedTagsKey);
        // empty as default value
        self::$aDefaultProperties[$prop] = '';
      }
    }
    ...
    if ( !isset(self::$_defaults) ) {
      ...
      // CSS Extended
      if (file_exists(dirname(__FILE__) . '/HTMLExtended.php') && HTML2WordML::$htmlExtended) {
        $cssExtendedInheritedTags = HTMLExtended::getCSSExtendedStylesInherited();
        foreach ($cssExtendedInheritedTags as $cssExtendedInheritedTag) {
          $inheritedTag = str_replace('-', '_', $cssExtendedInheritedTag);
          self::$_inherited[] = $inheritedTag;
        }
      }
    }
  }
   ...
   * @return string
   */
  function get_css_variable_value($value) {
    if (HTML2WordML::$parseCSSVars) {
      // parse CSS variables
      if (is_string($value) && substr($value, 0, 4) == 'var(') {
        $cssVar = str_replace('-', '_', substr(trim($value), 4, -1));
        if (isset($this->_props[$cssVar])) {
          $value = $this->_props[$cssVar];
        } else {
          // check if there's a default value
          $cssVarChunks = array_map('trim', explode(',', $cssVar, 2));
          if (count($cssVarChunks) > 1) {
            if (isset($this->_props[$cssVarChunks[0]])) {
              // get first value as CSS variable
              $value = $this->_props[$cssVarChunks[0]];
            } else {
              // get second value as value or as new var
              if (is_string($cssVarChunks[1]) && substr($cssVarChunks[1], 0, 4) == 'var(') {
                $cssVar = str_replace('-', '_', substr(trim($cssVarChunks[1]), 4, -1));
                if (isset($this->_props[$cssVar])) {
                  $value = $this->_props[$cssVar];
                } else {
                  // check if there's a default value
                  ...
                  if (isset($this->_props[$cssVarChunks[0]])) {
                    // get first value as CSS variable
                    $value = $this->_props[$cssVarChunks[0]];
                  } else if (isset($cssVarChunks[1])) {
                    $value = $cssVarChunks[1];
                  }
                }
              } else if (isset($cssVarChunks[1])) {
                $value = $cssVarChunks[1];
  ...
  function length_in_pt($length, $ref_size = null) {
    if (HTML2WordML::$parseCSSVars && isset($length)) {
      $length = $this->get_css_variable_value($length);
    }
    ...
    foreach ($length as $l) {
      ...
      if ( ($i = mb_stripos($l, "px"))  !== false ) {
        $ret += ((float)mb_substr($l, 0, $i)  * 72) / PARSERHTML_DPI;
        continue;
      }
  ...
  function inherit(StyleParser $parent) {
    ...
    if (HTML2WordML::$parseCSSVars) {
      // parse CSS variables from parent
      foreach ($parent->_props as $parentProsKey => $parentProsValue) {
        if (substr($parentProsKey, 0, 2) == '__') {
          if ( isset($parent->_important_props[$parentProsKey]) ) {
            $this->_important_props[$parentProsKey] = true;
          }
          $this->_props[$parentProsKey] = $parent->_props[$parentProsKey];
        }
      }
    }
  ...
  function munge_colour($colour) {
    ...
    // rgb( r,g,b ) and rgba( r,g,b,a ) formats
      ...
      // remove alpha transparency rgba
      if (count($triplet) == 4) {
        array_pop($triplet);
      }
    ...
    // hsl( h,s,l ) and hsla( h,s,l,a ) formats
    } else if ( mb_strpos($colour, "hsl") !== false ) {
      $i = mb_strpos($colour, "(");
      $j = mb_strpos($colour, ")");
      ...
      $tripletRGB = $this->colorHSLToRGB(trim($triplet[0])/360, str_replace('%', '', trim($triplet[1]))/100, str_replace('%', '', trim($triplet[2]))/100);


      foreach (array_keys($tripletRGB) as $c) {
        $tripletRGB[$c] = floor(trim($tripletRGB[$c]));
      }

      return $this->getArray(vsprintf("%02X%02X%02X", $tripletRGB));
    // cmyk( c,m,y,k ) format
    ...
    // parse CSS variables
    if (HTML2WordML::$parseCSSVars && substr($colour, 0, 4) == 'var(') {
      return $colour;
    }
  }
  ...
  function __set($prop, $val) {
    ...
    $this->_prop_cache[$prop] = null;
    if (HTML2WordML::$parseCSSVars) {
      // allow default properties and CSS variables
      if (!isset(self::$_defaults[$prop]) && !(substr($prop, 0, 2) == '__')) {
        global $_parserhtml_warnings;
        $_parserhtml_warnings[] = "'$prop' is not a valid CSS property.";
        return;
      }
    } else {
  ...
  function getpropstyle($prop) {
    if ( !isset(self::$_defaults[$prop]) ){
        //throw new Exception("'$prop' is not a valid CSS property.");
        return;
    }

    if (HTML2WordML::$parseCSSVars && isset($this->_props[$prop])) {
        $this->_props[$prop] = $this->get_css_variable_value($this->_props[$prop]);
    }
  ...
  function get_props($prop) {
    // parse CSS variables
    if (HTML2WordML::$parseCSSVars && $prop == 'font_family' && (isset($this->_props[$prop]) || isset($this->_prop_cache[$prop]))) {
      $newFontFamily = false;
      if (isset($this->_prop_cache[$prop])) {
        $newFontFamily = $this->get_css_variable_value($this->_prop_cache[$prop]);
      } else if (isset($this->_props[$prop])) {
        $newFontFamily = $this->get_css_variable_value($this->_props[$prop]);
      }
  ...
  {
      $letter_spacing = $this->_props["letter_spacing"];

      if ($letter_spacing === "normal" || $letter_spacing === '') {
          return 0;
      }

      return $this->length_in_pt($letter_spacing, $this->get_font_size());
  }
  ...
  */
  ...
  protected function _set_border($side, $border_spec, $important) {
    ...
    if (HTML2WordML::$parseCSSVars) {
      // parse CSS variables
      if (isset($arr[0]) && ( (preg_match("/[.0-9]+(?:px|pt|pc|em|ex|%|in|mm|cm)|(?:thin|medium|thick)/", $arr[0] ) && !strstr($arr[0], 'hsl(') ) || substr($arr[0], 0, 4) == 'var(')) {
        $this->_set_style_side_type('border',$side,'_width',$arr[0],$important);
      }
      if (isset($arr[1]) && (in_array($arr[1], $BORDER_STYLES) || substr($arr[1], 0, 4) == 'var(')) {
        $this->_set_style_side_type('border',$side,'_style',$arr[1],$important);
      }
      if (isset($arr[2])) {
        $this->_set_style_side_type('border',$side,'_color',$arr[2],$important);
      }
    } else {
      foreach ($arr as $value) {
        ...
        } else if ( preg_match("/[.0-9]+(?:px|pt|pc|em|ex|%|in|mm|cm)|(?:thin|medium|thick)/", $value ) && !strstr($value, 'hsl(') ) {
...
class StyleParsersheet {
  ...
  /**
   * Keep font-face fonts
   */
  private $_font_face_data = array();
   ...
   * @return array
   */
  function get_font_face_data() { return $this->_font_face_data; }
...
//    }
  ...
  private function _parse_font_face($str) {
    if (file_exists(dirname(__FILE__) . '/HTMLExtended.php') && HTML2WordML::$htmlExtended) {
      ...
      $this->_font_face_data = array_merge($this->_font_face_data, $sources);
    }
  }
