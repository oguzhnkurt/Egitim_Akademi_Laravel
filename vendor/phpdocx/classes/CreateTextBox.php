...
class CreateTextBox extends CreateElement
{
    ...
    private function generateSHAPE($options, $width, $height)
    {
        ...
        $shapeId = rand(1000, 999999);
        $strokeStyle = 'joinstyle="miter"';
        if (isset($options['dashStyle'])) {
            $strokeStyle .= ' dashstyle="' . $options['dashStyle'] . '"';
        }
        if (isset($options['lineStyle'])) {
            $strokeStyle .= ' linestyle="' . $options['lineStyle'] . '"';
        }
        $shape = '<v:shapetype coordsize="21600,21600" id="_x0000_t202" o:spt="202" path="m,l,21600r21600,l21600,xe"><v:stroke '.$strokeStyle.'/><v:path gradientshapeok="t" o:connecttype="rect"/></v:shapetype>';
        $shape .= '<v:shape id="_x0000_s' . $shapeId . '" type="#_x0000_t202" style="';
        if (in_array($options['align'], $align)) {
            $shape .= 'position:absolute;';
            $shape .= 'mso-position-horizontal:' . $options['align'] . ';';
        }

        if (isset($options['position']) && $options['position'] == 'absolute' && isset($options['align']) && $options['align'] == 'absolute') {
            $shape .= 'position:absolute;mso-position-horizontal:absolute;mso-position-vertical:absolute;';
        }
        if (isset($options['marginTop'])) {
            $options['margin_top'] = $options['marginTop'];
        }
        if (isset($options['marginLeft'])) {
            $options['margin_left'] = $options['marginLeft'];
        }
        if (isset($options['marginBottom'])) {
            $options['margin_bottom'] = $options['marginBottom'];
        }
        if (isset($options['marginRight'])) {
            $options['margin_right'] = $options['marginRight'];
        }
        ...
        if (isset($options['z-index'])) {
            $shape .= 'z-index:' . $options['z-index'] . ';';
        } else {
            $shape .= 'z-index:' . rand(11111111, 999999999) . ';';
        }

        if (isset($options['relativeToHorizontal'])) {
            $shape .= 'mso-position-horizontal-relative:' . $options['relativeToHorizontal'] . ';';
        }

        if (isset($options['relativeToVertical'])) {
            $shape .= 'mso-position-vertical-relative:' . $options['relativeToVertical'] . ';';
        }

        if (isset($options['contentVerticalAlign']) && in_array($options['contentVerticalAlign'], $vAlign)) {
