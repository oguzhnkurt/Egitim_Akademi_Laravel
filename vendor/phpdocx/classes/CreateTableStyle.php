...
class CreateTableStyle
{
    ...
    public function addTableStyle($name, $styleOptions)
    {
        ...
        if (isset($styleOptions['next']) && $styleOptions['next']) {
            $tableStyle .= '<w:next w:val="'.$styleOptions['next'].'"/>';
        }
        ...
        if (isset($styleOptions['semiHidden']) && $styleOptions['semiHidden']) {
            $tableStyle .= '<w:semiHidden/>';
        }
        if (isset($styleOptions['unhideWhenUsed']) && $styleOptions['unhideWhenUsed']) {
            $tableStyle .= '<w:unhideWhenUsed/>';
        }

        if (isset($styleOptions['pPrStyles'])) {
            $stylepPrOptions = CreateDocx::translateTextOptions2StandardFormat($styleOptions['pPrStyles']);
            $newStyle = new CreateParagraphStyle();
            $newStyle->addParagraphStyle(uniqid((string)mt_rand(999, 9999)), $stylepPrOptions);

            $tableStyle .= $newStyle->XMLPPr();
        }

        if (isset($styleOptions['rPrStyles'])) {
            $stylerPrOptions = CreateDocx::translateTextOptions2StandardFormat($styleOptions['rPrStyles']);

            $newStyle = new CreateParagraphStyle();
            $newStyle->createCustomCharacterStyle(uniqid((string)mt_rand(999, 9999)), $stylerPrOptions);
        ...
        // tcPr styles
        if (isset($styleOptions['cellBackgroundColor']) || isset($styleOptions['cellVerticalAlign'])) {
            $tableStyle .= '<w:tcPr>';

            if (isset($styleOptions['cellBackgroundColor'])) {
                $tableStyle .= '<w:shd w:fill="'.$styleOptions['cellBackgroundColor'].'" w:val="clear"/>';
            }

            if (isset($styleOptions['cellVerticalAlign'])) {
                $tableStyle .= '<w:vAlign w:val="'.$styleOptions['cellVerticalAlign'].'"/>';
            }
