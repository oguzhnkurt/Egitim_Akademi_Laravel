...
if ($isWeb) {
    $output .= '<html>';
    $output .= '<head>';
    $output .= '<title>phpdocx checker</title>';
...
if ($iPhpVersion < 50600) {
    ...
    $output .= 'Your PHP version (' . PHP_VERSION . '), is too old, please update to PHP 5.6 or newer.' . $break;
} else {
	if ($isWeb) {
...
if (!extension_loaded('gd')) {
    ...
    $output .= 'Please install and enable the GD extension to add images.' . $break;
} else {
    if ($isWeb) {
        $output .= '<span class="testok">';
    }
...
if (!extension_loaded('exif')) {
    ...
    $output .= 'Please install and enable the exif extension to add images from HTML.' . $break;
} else {
    if ($isWeb) {
        $output .= '<span class="testok">';
    }
...
if (!extension_loaded('imagick')) {
    ...
    $output .= 'Please install and enable the imagick extension to add SVG images.' . $break;
} else {
    if ($isWeb) {
        $output .= '<span class="testok">';
    }
    ...
    $output .= 'imagick is enabled.' . $break;
}
if ($isWeb) {
    $output .= '</li>';
}
...
$output .= 'SERVER_NAME: ' . (isset($_SERVER['SERVER_NAME']) ? $_SERVER['SERVER_NAME'] : '') . "\n";
$output .= 'SERVER_SOFTWARE: ' . (isset($_SERVER['SERVER_SOFTWARE']) ? $_SERVER['SERVER_SOFTWARE'] : '') . "\n";
$output .= 'SERVER_ADDR: ' . (isset($_SERVER['SERVER_ADDR']) ? $_SERVER['SERVER_ADDR'] : '') . "\n";
$output .= 'SERVER_PROTOCOL: ' . (isset($_SERVER['SERVER_PROTOCOL']) ? $_SERVER['SERVER_PROTOCOL'] : '') . "\n";
$output .= 'HTTP_HOST: ' . (isset($_SERVER['HTTP_HOST']) ? $_SERVER['HTTP_HOST'] : '') . "\n";
$output .= 'HTTP_X_FORWARDED_FOR: ' . (isset($_SERVER['HTTP_X_FORWARDED_FOR']) ? $_SERVER['HTTP_X_FORWARDED_FOR'] : '') . "\n";
$output .= 'PHP_SELF: ' . (isset($_SERVER['PHP_SELF']) ? $_SERVER['PHP_SELF'] : '') . "\n";
